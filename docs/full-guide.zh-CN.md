...
（`OPA001`-`OPA010`）：dry-run 派发屏障、禁止主工作区 fallback 实现、heartbeat
...
派发，以及没有命令、diff、gate 或 artifact 证据绑定的完成/自述声明。这些发现只是本地静态疑点，不是最终判定。它不会 stage、commit、merge、push、tag、release、清理 worktree、派发
