...
7. 当项目需要持久状态时，Codex 可以运行
   ...
   orchestration policy、thread map、concepts、inbox 和 pulse/inbox/router
   prompts 的本地模板。
...
简洁 project map。
同时也会展示只读 `threadMap`、`concepts` 和 `inbox` 信号，提醒长期线程拓扑、
项目概念库和反馈收件箱是否已经落到本地文件，而不是只存在聊天记忆里。
...
matrix、contract checklist、restart policy、subjective rubric、exit condition、
blocked condition、waivers、decision trace 和 SOP feedback。重要功能包派 worker 前应该先创建
...
`context --write-file .codex-orchestrator/context.md` 会把当前 ledger/status/package
truth 投影成一份很短的接手包，给下一轮 Codex 或新 session 读。它会列出优先阅读文件、
当前主线、任务计数、下一步、restart policy、contract policy，以及 UI/体验/parity
任务可用的主观评分 rubric。它比完整状态页短，适合上下文压缩、handoff、或长 `/goal`
运行前刷新。它只是 local/static 接手辅助，不替代 repo truth 检查。
...
无人值守夜跑和功能包收口时，本地 review pack 不能单独算 durable 项目记录。如果异模型
review 影响了统领的 accept / reject / waiver / package closed 决策，就要在目标项目的
持久 review/progress 表面提交一份短 summary，例如 `docs/reviews/`。这份 summary 至少
写清 reviewer、状态、已知 P0/P1/P2/P3 数量、已接受修复或 waiver、剩余 advisory
风险，以及 `proxy/advisory` 证据标签。原始 `.codex-orchestrator/` pack 可以继续保持
本地未跟踪，但功能包为什么能收口，不能只存在本机控制面目录或工作区清理前的临时文件里。

reviewer 超时也要变成显式状态，而不是一直等。`review run` 支持正常使用的
`--timeout-minutes`，也支持短 smoke / 测试用的 `--timeout-seconds`。超时会记录成
`blocked` reviewer timeout，并保持 `proxy/advisory` 证据口径。统领接下来应该选择：
加大 timeout 重跑、导入另一个 reviewer 的结果，或者在项目 policy 允许时明确记录
optional-skipped / waiver；不要因为一个 reviewer 不可用就无限期卡住整个 package
收口。
...
对于事故或生产类 bugfix package，默认把 AI 循环停在发布边界之前：收集结构化日志、
还原复现步骤、能写就补 failing test 或 fixture、准备有界修复分支，并跑回归门禁。
除非项目对这一个动作有明确授权，否则统领或 worker 不能修改生产数据、部署、回滚、
切换 provider/payment 行为，或执行破坏性修复。
