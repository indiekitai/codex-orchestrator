...
1. Ledger-Enforced Dispatch Closure：待做。
   ...
   - 推荐命令形态：
     ...
     和 `codex-orchestrator dispatch reconcile --task-id TASK`
   - 边界：local/static orchestration state；pendingWorktreeId 不是 running worker
     proof；resolved worktree 不是 task correctness proof。

2. Merge-Readiness Pack：待做。
   - 目标：把一个 completed-unreviewed task 转成标准验收包。
   - 输出 task/branch/worktree/base commit、diff name-status、allowed/forbidden
     path、`git diff --check`、review doc/artifact/self-review/evidence label/docs
     drift、gates 和 residual risk。
   - 推荐命令：
     `codex-orchestrator pack merge-readiness --task-id TASK`
   - 边界：local/static review evidence；不自动 merge/push/cleanup，不声称 runtime
     或 production proof。

3. Project-Aware Roadmap Scorer：待做。
   ...
   - 边界：local/static planning evidence；只建议 dispatch，不代表允许 merge。

4. Consultation Request Pack：待做。
   ...
   - 边界：默认 `blocked`，辅以 local/proxy/direct 证据；不隐藏不确定性。

5. Transcript / Heartbeat Failure Eval：待做。
   - 目标：把真实编排失败变成 regression fixture。
   - 覆盖 child 完成后忘记继续队列、heartbeat 绑定旧任务、pendingWorktreeId 被误当
     running worker、setup 失败后 fallback 主 checkout、证据升级、统领下场写 worker
     代码等错误。
   - 推荐命令：
     `codex-orchestrator eval run --suite orchestration-transcripts`
   - 边界：local/static eval；不解析私有 transcript 时必须标 blocked 或使用脱敏 fixture。

6. Static Dashboard / Status Page：待做。
   ...
   - 边界：local/static status evidence；不引入 daemon/web server。
