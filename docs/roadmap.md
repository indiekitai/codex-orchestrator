...
下一批应该优先做这些功能包，而不是继续补零散 routine：

0. Misalignment Event Log / Pushback Capture：待做。
...
0.5 Claim Verifier / Evidence-Bound Self-Report：待做。
...
0.6 Misalignment Taxonomy Policy/Eval Fixtures：待做。
...
0.7 Trust-Risk Status Block：待做。
...
0.8 Constraint Stack / Worker Contract Snapshot：待做。
...
0. Model-Agnostic Review Pack：已补 package-level external review MVP。
   ...
   - 已解决的第三层问题：`packageSummary` 现在会根据内置 review policy 标记
