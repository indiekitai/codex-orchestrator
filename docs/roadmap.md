...
2.1 Context projection for long sessions：已补第一轮 helper artifact。
   - 目标：把完整 transcript、ledger、events、status、review reports 和 artifacts
     投影成下一轮 model call 真正需要看的短上下文，而不是持续 append 全部历史。
   - 当前落地：新增 `codex-orchestrator context --write-file .codex-orchestrator/context.md`
     和 JSON report。它输出 current lane、dispatch mode、active/review/blocked/cleanup
     计数、优先阅读文件、下一步、restart policy、contract policy，以及 UI/体验/parity
     任务的 subjective rubric。SKILL 要求 heartbeat/resume/long `/goal` 前刷新这份短
     context pack。
   - 同步落地：`pack spec` 模板新增 Contract checklist、Restart policy 和
     Subjective rubric；`pack eval` 的 stop conditions 也加入 restart 语义。
...
8. Thread topology / Pulse-Router-Inbox package：已完成。
   - 目标：吸收 Codex power-user 的 Pulse / Log / Inbox / Router 长期线程模式，
     但保持 codex-orchestrator 的项目工程编排边界。
   - 当前落地：`init --write-templates` 生成 `thread-map.md` 和
...
- worker prompt 模板可以拆成可复制的 snippets，并把 minimum-change / challenge
  contract / spec-gates-first 作为默认片段。
...
`OPA001`-`OPA011` 覆盖 dry-run 派发屏障、主工作区 fallback guard、heartbeat
...
派发、没有证据绑定的完成/自述声明，以及 Router/Inbox/Pulse/Log 协调线程
越权执行。heartbeat lifecycle guard 还覆盖已验证通用 monitor 每轮反复写入当前
...
budget-policy helper 控制或证据夸大、从全局安全 backlog 补两个互不相关任务
导致产品包主线断裂，以及 Router/Inbox/Pulse/Log 被写成可以直接实现、派发、
merge、push 或 cleanup 的执行线程。
...
0. Model-Agnostic Review Pack：已补 package-level external review MVP。
   ...
   - 已解决的第四层问题：夜跑或重要功能包收口时，外部 review 不能只留在本机
     `.codex-orchestrator/` raw pack 里。SKILL 和完整指南现在要求：如果异模型 review
     影响 accept / reject / waiver / package closed 决策，统领要把 reviewer、状态、
     finding counts、已采纳修复或 waiver、剩余 advisory 风险和 `proxy/advisory`
     证据标签写入目标项目的 durable review/progress 表面，例如 `docs/reviews/`。
     本地 pack 仍可不跟踪，但收口理由必须能被新统领 session 和 repo 历史恢复。
   ...
   - 已解决的第五层问题：`packageSummary` 现在会根据内置 review policy 标记
...
6.5 Minimum-change and spec-first worker contracts：已补规则层。
   - 目标：减少 AI worker 在已有局部闭合的领域里继续新增页面、抽象、shell 或
     readiness 表面，先证明为什么这次需要写代码。
   - 当前落地：SKILL 要求派发前执行 minimum-change gate；重要 package 先写
     package intent 和 gates；worker 开始实现前必须 challenge 合同，若合同清楚则写
     contract accepted 和最小改动理由；self-review 也要检查 minimum-change rationale。
   - 边界：这是 prompt/runbook 约束，还不是 helper 自动判定。后续如果真实项目继续
     出现“能复用却新增代码”或“worker 盲从过宽合同”的问题，再考虑做成 policy/eval
     fixture 或 prompt snippet generator。
