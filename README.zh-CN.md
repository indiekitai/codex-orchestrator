...
在这个项目里，harness 不是 MCP、skills、schema 或辅助命令的简单集合。这些东西都很有用，
但它们只是局部能力。

harness 真正要做的是，把一个项目里“什么算对”变成 Agent 能读取、执行和检查的流程：

- 哪些源码、文档和历史决策算作上下文；
- worker 可以使用哪些工具、触碰哪些路径；
- 输出必须符合什么形状，必须跑哪些 gate，证据要怎么标；
- 什么情况下必须停在 `blocked`，而不是自己猜；
- review 反馈应该如何沉淀成规则、fixture、spec 或状态页改进。

如果这些规则没有写下来，Agent 就会借用训练数据里的通用默认值，做出“看起来很完整、
但不符合这个项目”的结果。`codex-orchestrator` 的价值，就是把这些项目自己的判断标准放进循环里。
...
- **文档和模板**：项目地图、功能包规格、评估矩阵、功能包计划、编排策略、
  线程地图、概念库/收件箱、Pulse/Inbox/Router 提示词、案例和 routine 规范。
